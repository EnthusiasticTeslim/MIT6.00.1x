    ...
    def apply_shift(self, shift):
        ...
        for ch in self.message_text:
            if ch not in invalid:
                self.cipherText += self.cipher[ch]
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        '''
        # possible shift range
        mins = 0
        maxs = 26
        # Tracking the possible shift value
        shift_tracker = 0
        # Tracking the maximum valid words 
        max_word = 0
        possible_shift = range(mins, maxs + 1)
        
        # Validating the possible shift i.e. key values
        for shift_guess in possible_shift:
            # checking the number of correct words
            word_len = 0
            decrypt_guess = Message.apply_shift(self,shift_guess).split(' ')
            for formed_words in decrypt_guess: # loop through the word in the cipherText using the guess
                if is_word(self.valid_words, formed_words): # if valid, returns True
                    word_len += 1
            
            # storing the max possible words counts and corresponding shift value
            if word_len > max_word:
                max_word = word_len
                shift_tracker = shift_guess
                
        plaintext = Message.apply_shift(self,shift_tracker)
        return (shift_tracker,plaintext)
                
def decrypt_story():
    '''
    For this problem, this will use our implementation of the Message, 
    PlaintextMessage, and CiphertextMessage classes
    Now that you have all the pieces to the puzzle, please use them to decode the file story.txt. 
    Use helper function get_story_string() that returns the encrypted version of the story as a string. 
    ...
    '''
    # store the text in message_encrypt
    message_encrypt = get_story_string()
    # Call the object CiphertextMessage
    Ciphering = CiphertextMessage(message_encrypt)
    return Ciphering.decrypt_message() # decrypt the CipherText
